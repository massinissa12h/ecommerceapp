...
export function UserAvatar({ user, className, linkToId }: UserAvatarProps) {
  ...
  const inner = (
    <Avatar className={cn('size-10 ring-2 ring-primary/10 transition-transform hover:scale-105', className)}>
      {user.avatar_url ? <AvatarImage src={user.avatar_url} alt={userDisplayName(user)} /> : null}
